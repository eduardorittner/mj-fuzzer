...
# Import BNF AST types for adapter function
from bnf_ast import NonTerminal as BnfNonTerminal, Rule as BnfRule, SymbolReference as BnfSymbolReference, SymbolType, Modifier
...
def convert_bnf_symbols(bnf_symbols: Dict[str, BnfNonTerminal]) -> Dict[str, Symbol]:
    """
    Convert BNF AST symbols to fuzzer symbols with generate methods
    
    :param bnf_symbols: Dictionary of BNF AST symbols
    :return: Dictionary of fuzzer symbols
    """
    fuzzer_symbols: Dict[str, Symbol] = {}
    
    # First pass: Create all terminal symbols
    terminals_created: set[str] = set()
    for bnf_symbol in bnf_symbols.values():
        for rule in bnf_symbol.rules:
            for symbol_ref in rule.symbols:
                # Handle SymbolReference objects
                if isinstance(symbol_ref, BnfSymbolReference):
                    if symbol_ref.ty == SymbolType.TERMINAL and symbol_ref.name not in terminals_created:
                        fuzzer_symbols[symbol_ref.name] = Terminal(symbol_ref.name)
                        terminals_created.add(symbol_ref.name)
                # Handle Modifier objects (we'll treat the symbols inside modifiers as regular symbols)
                elif isinstance(symbol_ref, Modifier):
                    for inner_symbol in symbol_ref.symbols:
                        if inner_symbol.ty == SymbolType.TERMINAL and inner_symbol.name not in terminals_created:
                            fuzzer_symbols[inner_symbol.name] = Terminal(inner_symbol.name)
                            terminals_created.add(inner_symbol.name)
    
    # Second pass: Create all non-terminal symbols
    for name, bnf_symbol in bnf_symbols.items():
        production_rules: List[ProductionRule] = []
        for rule in bnf_symbol.rules:
            # Convert symbol references to symbol names
            symbol_names: List[str] = []
            for symbol_obj in rule.symbols:
                # Handle SymbolReference objects
                if isinstance(symbol_obj, BnfSymbolReference):
...
class Fuzzer:
    ...
    def from_bnf_parser(cls, bnf_parser: 'BnfParser', max_depth: int = 10):
        ...
        """
        # Convert BNF symbols to fuzzer symbols
        fuzzer_symbols = convert_bnf_symbols(bnf_parser.symbols)
        return cls(fuzzer_symbols, max_depth)
    ...
    def _calculate_distance_to_terminals(self, symbol_name: str, visited: Optional[set[str]] = None) -> int:
        """
        Calculate the minimum distance from a symbol to terminal symbols using DFS.
        
        :param symbol_name: Name of the symbol to calculate distance for
        ...
        if isinstance(symbol, NonTerminal) and symbol.rules:
            ...
            for rule in symbol.rules:
                max_rule_distance = 0
                for rule_symbol_name in rule.symbols:
                    # Calculate distance for this symbol and add 1 for the expansion step
                    distance = self._calculate_distance_to_terminals(rule_symbol_name, visited.copy()) + 1
    ...
    def _choose_best_rule_at_limit(self, symbol: NonTerminal, symbol_name: str) -> ProductionRule:
        """
        ...
        for rule in symbol.rules:
            max_distance = 0
            for rule_symbol_name in rule.symbols:
                distance = self._calculate_distance_to_terminals(rule_symbol_name)
        ...
        best_rules = [rule for rule, distance in zip(symbol.rules, rule_distances) if distance == min_distance]
